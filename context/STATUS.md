...
1. **✅ SHIPPED TO PROD — Document Upload feature + UX polish (commits c21949f + ccd655a)** — drag-and-drop PDF/DOCX/TXT into chat, filtered by existing PII pipeline, sticky across follow-ups. Live on https://simah-ai-search-control.vercel.app as of 2026-04-09. **Final state:** 86 tests pass across 5 test files, build clean, TypeScript clean, zero new lint errors. **Bonus fixes shipped in same session:** IBAN hotfix (real 24-char Saudi IBANs now caught), greeting fast-path (hi/hello/salam return canned bilingual reply, no LLM cost), classifier-skip when document attached (was rejecting "read this"), granular PDF errors (encrypted/invalid/format), Enter-to-confirm in preview modal, chip-disappears-after-send UX (matches ChatGPT/Claude), Turbopack `serverExternalPackages` config for pdf-parse/pdfjs-dist/mammoth (was breaking PDF extraction in dev). All 4 wave RESULT files in `context/builds/`. **Vercel deploy gotcha learned (re-confirms STATUS decision #8):** the first deploy attempt failed because the commit author was `eugenezhang@eugenezhangco-spec.com` instead of `aiagent@qarar.org`. Fixed with an empty trigger commit (`ccd655a`) authored as QararAIAgent. The error message from `vercel --prod --yes` was empty — the real reason was buried in the Vercel API at `readyStateReason` ("Git author ... must have access to the team qararaiagent's projects"). When this happens again, run: `git -c user.name="QararAIAgent" -c user.email="aiagent@qarar.org" commit --allow-empty -m "chore: trigger Vercel deploy"`.
